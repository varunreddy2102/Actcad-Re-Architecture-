...
- **Performance ceiling on large drawings.** Public reviews repeatedly cite "moderately large drawings frequently get stuck" — symptomatic of the synchronous main-thread command/render loop typical of the IntelliCAD shell, combined with no enforced memory budget, eager kernel materialization, and linear-scan hit-testing. The re-architecture addresses this with a memory architecture committed up-front rather than retrofitted — strategic picture in `docs/rearchitecture-plan.md` §16; full engineering reference in **`docs/memory-architecture.md`** (five-layer model, eight commitments, MVCC concurrency, KAL contract, eviction protocol, failure modes, CI gates, anti-patterns, IntelliCAD-migration teaching).
